...
### Fixed
- **Phase 2 dialogue (`claude-chatgpt-dialogue.mjs`) no longer hard-fails when `CLAUDE_CODE_OAUTH_TOKEN` is unset.** The `claude-responds` job previously exited immediately if the OAuth token was missing or expired, even when a standard API key was available — leaving ChatGPT's review threads un-triaged and blocking the merge-gate. A new pure `resolveAnthropicAuth()` helper in `lib/anthropic-auth.mjs` prefers the OAuth token (`Authorization: Bearer`) and falls back to `ANTHROPIC_API_KEY` (`x-api-key`), failing only when neither is set, and logs which credential is in use. The `chatgpt-pr-review.yml` `claude-responds` job now also passes the `ANTHROPIC_API_KEY` secret. To avoid surprise costs, the model now defaults per auth source via `selectModel()`: Opus (`claude-opus-4-7`) on the subscription-covered OAuth path, but the cheaper `claude-haiku-4-5` on the per-token API-key path (a `MODEL` env var still overrides either). The credentials now form an ordered chain (`resolveAnthropicAuthChain()`): the request tries the OAuth token first and, on a `401`/`403` (`isAuthFailureStatus()`), automatically retries once with the API key — so an *expired-but-present* OAuth token is recovered instead of failing the job. The reply/synthesis attributions name whichever model actually responded. Covered by 18 unit tests in `test/anthropic-auth.test.mjs`.
