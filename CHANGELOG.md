...
### Added
- **Stale epics can now be archived out of the epic pickers with one click (⚙ → 🧹 tidy)** — the epic quick-selector in the log entry had grown to 26 epics, 13 of which had no log entry and no board task in the last three weeks, making the epic for the task actually in hand hard to find among long-finished ticket epics. `🧹 tidy` scans every epic against a 21-day activity window (log entries *and* board tasks both count as use), lists what it would archive, and on confirmation stamps `archived: true` on each one. Archived epics disappear from all six pickers — the tag-row dropdown, the log-entry chip row, the board task card, the hero menu, the rapid-capture filter chips, and the tracker form — while `getCat()` still resolves their label and colour, so past entries, timesheets, and the weekly/monthly reports render exactly as before. **Deliberately not a delete:** the existing `× delete` button hard-removes an epic, and every historical entry pointing at it silently falls back to the grey `other` category, which would have retroactively untagged three weeks of history. Built-in epics (`work`, `meeting`, `deep focus`, `break`, `other`) and the currently selected epic are never archived; `other` in particular doubles as `getCat()`'s fallback. Anything archived by mistake comes back from `⚙ → 🗃 N archived`, and a Jira import that lands a task on an archived epic un-archives it automatically — an imported ticket is proof the epic is live again, and leaving it hidden would tag the new task to an epic the user cannot see. That revival deliberately happens in the import handler, next to the existing `save()`, rather than in the epic-matching lookup: matching runs while the CSV is still only being *previewed*, so un-archiving there would mutate persisted state for an import the user might abandon, and would also revive epics whose tickets they had deselected. Decision logic lives in a new pure leaf module, `src/js/pure-fns-epics.js` (`findStaleCategories`, `pickableCategories`, `applyEpicArchive`, `restoreArchivedCategory`, `epicCutoffDate`, `collectRecentlyUsedCatIds`), covered by 18 unit tests — including the window boundary (an epic used exactly 21 days ago is kept, 22 days is archived), undated records not counting as recent, and tidying twice being a no-op — plus 6 integration tests driving the real tag-row handlers in `test/unit/utils-categories.test.mjs`.
