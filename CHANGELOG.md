...
### Fixed
- **`auto-chore.yml` couldn't push a fix that touched a workflow file, and failed silently when it couldn't** — found by dogfooding the workflow on a real test issue (#297) asking it to modify `auto-chore.yml` itself: the default `GITHUB_TOKEN` refused the push ("refusing to allow a GitHub App to create or update workflow ... without `workflows` permission"), and because that push step failed, the "comment on the issue with the outcome" step was skipped too (GitHub Actions skips subsequent steps by default after a failure) — leaving the issue with zero visibility into what happened. Added `workflows: write` to the job's permissions, and the comment step now runs regardless of an earlier step's outcome (`!cancelled()`), with a new distinct message for "committed but failed to push" versus the existing "declined to automate" and "CLI itself crashed" cases.
