...
### Added
- **README's "Project documentation" table now links the generated JSDoc reference site (#337)** — `docs.yml` has deployed working API docs to `https://lupinesse.github.io/work-log/` on every push to `main` for a while (verified live, 200 OK), but nothing in the repo's own documentation pointed to it — anyone browsing `README.md`/`ARCHITECTURE.md`/`CONTRIBUTING.md` had no way to discover it existed. Flagged as an audit finding in #337's original workflow-output review; fixed now. Separately, that same review flagged `delivery-metrics.yml` as suspicious — 10 of 12 weekly runs between 2026-06-01 and 2026-08-03 produced no output. Re-investigated: this was a real, already-diagnosed-and-fixed bug, not a new one — PR #288 (merged 2026-08-03) found the root cause ("9 weeks of silent failures" from pushing straight to protected `main` instead of via branch+PR) and fixed it; the two runs since (2026-08-10, 2026-08-17) both correctly produced a PR. No further action needed there.
