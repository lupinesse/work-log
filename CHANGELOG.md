...
### Changed
- **Second ES-module extraction: `isToday`/`fmtLabel` pulled out of `02-utils.js` into a new `date-labels.js` leaf module (#336)** — `02-utils.js` as a whole was already ruled out as an extraction candidate (too entangled with app state and later-loaded render functions), but two of its functions were genuinely stateless: `isToday(d)` and `fmtLabel(d)` only call `dk()` (from the `pure-fns.js` leaf module) and JS builtins, with zero references to `categories`/`selectedTag`/other module state. Widely used — 8 `src/js` files call them. Moved to `date-labels.js`, imported at the top of the bundle like the other leaf modules; `date-labels.js` itself imports `dk` from `pure-fns.js` explicitly (it's a real standalone ES module, so it can't rely on the flat bundle's ambient scope the way `02-utils.js` still does). `build.js`/`vite.config.js`'s per-leaf-module import-line generation — until now three near-identical hardcoded blocks (`pure-fns.js`, `logger.js`, `app-constants.js`) — was generalised into a loop over `LEAF_MODULES` (excluding the `pure-fns-*.js` barrel sub-modules, which are only ever consumed via the `pure-fns.js` barrel itself) so a 4th leaf module doesn't need a 4th hand-copied block. Neither function had dedicated test coverage before (other test files only ever re-implemented a stub version for their own sandbox setup) — 10 new regression tests added in `test/unit/date-labels.test.mjs`, testing the real functions directly since they're now real ES module exports. No call-site behaviour changes. Verified with `npm run build && npm test && npm run lint`, a manual `npm run portable` (syntax-checked, confirmed `isToday`/`fmtLabel` correctly inlined with no leftover `import`/`export`), and a real-browser check confirming `#dateLabel` still renders "today" via `fmtLabel(viewDate)`.
...
### Fixed
- **`auto-chore` told you to read a file it never printed** — found by the new workflow guard while fixing the `scss-audit`/`dead-code` outage, and the same invisible-failure bug in a milder form. The step captured the CLI exit code correctly and deliberately kept going (a later step fails the job, after the outcome has been commented on the issue), but the only thing it ever did with `chore-output.txt` was `wc -l` it — which reports a line count and prints none of the lines. So when the CLI genuinely errored, the job failed with `::error::… see chore-output.txt in this run's logs`, pointing at a file that had never reached the logs at all. The step now prints the output in a collapsible `::group::` on the failure path (with an explicit empty-file message for the case where the CLI died before writing anything), and the annotation was reworded to point at that group instead of at the unreachable file. Still does not exit non-zero itself — the existing two-stage comment-then-fail flow is deliberate and unchanged.
- **The `scss-audit` and `dead-code` workflows died ~1.6s into every run, and the reason was sealed in a file nothing ever read** — both invoke `claude -p` behind a credential-selection block that did the exact opposite of what its own header comment promised: it `unset CLAUDE_CODE_OAUTH_TOKEN` whenever `ANTHROPIC_API_KEY` was present, making the pay-as-you-go API key primary and the subscription token a fallback that could never be reached. When that key ran out of credit — between the last green runs, `dead-code` on 2026-08-10 and `scss-audit` on 2026-08-15 — every run began exiting 1 with `Credit balance is too low`. `pr-review.yml` never referenced `ANTHROPIC_API_KEY`, kept authenticating over OAuth, and passed throughout, which is what isolated the fault to credential selection rather than to the `--model claude-sonnet-4-6` argument the three workflows share; that model ID was checked against the current model line, is still valid, and was deliberately left alone. Selection is now OAuth-first with the API key as the documented fallback, and the branch taken is echoed to the log so the credential in use is never again a guess. **The deeper bug was that none of this was visible.** `scss-audit.yml` redirected both streams into `scss-output.txt`, and under `bash -e` a non-zero exit aborted the step before the "post report as PR comment" step could read that file, so the job surfaced a bare "Process completed with exit code 1" and nothing else — which is why a workflow that only triggers on `src/css/**` sat broken for six days without anyone noticing. Both workflows now capture the exit status explicitly instead of letting `bash -e` abort — `$?` for the redirect in `scss-audit`, `${PIPESTATUS[0]}` for the new `tee` pipeline in `dead-code`, where a plain `$?` would report tee's status and mask every failure — print the output in a collapsible `::group::` with an `::error::` annotation, and upload it via `actions/upload-artifact` so a long report is not truncated in the web log view. Verified by replaying both step bodies verbatim under `bash -e` against stub CLIs across the failure, success, and OAuth-absent paths, confirming the real `Credit balance is too low` text now reaches the log while the step still correctly exits non-zero.
...
### Added
- **A CI guard that fails the build when a workflow repeats the invisible-`claude`-failure pattern** — the two bugs behind the `scss-audit`/`dead-code` outage were both invisible by construction, so fixing the two files was not enough: nothing stopped the next workflow making the same mistake. `.github/scripts/lib/claude-cli-workflow-guards.mjs` adds pure, dependency-free predicates over a workflow's YAML source, and the companion test applies them to **every** workflow in `.github/workflows/` rather than to a hard-coded list, so a newly added one is covered automatically. Three properties are enforced for any step that runs `claude -p`: it must not `unset CLAUDE_CODE_OAUTH_TOKEN`; if it sends output to a file it must survive a non-zero exit (`status=$?`, `${PIPESTATUS[0]}`, or `|| true`) rather than letting `bash -e` abort the step; and something in that workflow must actually print the captured file. The abort check is step-scoped and the reveal check is workflow-scoped — that distinction is load-bearing, because `auto-fix-ci.yml` legitimately reveals its output from a *later* step (`tail -200 fix-output.txt` into a PR comment), and a step-scoped reveal check flagged it as a false positive until the scope was corrected. `wc -l` deliberately does not count as revealing: it reports a line count and shows none of the lines, which is exactly how the `auto-chore` diagnostic stayed hidden. Verified as a genuine regression test by running the guards against the pre-fix workflows straight out of `main`: 4 violations there, 0 on this branch, with `auto-fix-ci.yml` and `pr-review.yml` correctly clean in both. 31 unit tests, with the three real shipped shapes as fixtures. The suite also asserts the scan found several CLI-invoking workflows, so a broken scan cannot pass by silently checking nothing.
- **README's "Project documentation" table now links the generated JSDoc reference site (#337)** — `docs.yml` has deployed API docs to `https://lupinesse.github.io/work-log/` on every push to `main` since the workflow landed on 2026-05-25, but nothing in the repo pointed to them, so anyone browsing `README.md`/`ARCHITECTURE.md`/`CONTRIBUTING.md` had no way to discover the site existed. Raised as an audit finding in #337's workflow-output review.
