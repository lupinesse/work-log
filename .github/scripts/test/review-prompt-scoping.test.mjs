 ...
 *
 * Splits only on paragraph breaks and `. ` (a full stop). A bare `?` or `!`
 * is deliberately not treated as a statement boundary: both source files
 * phrase the rule as a rhetorical question ("never a silent catch?") whose
 * qualifying clause follows in the same sentence, and splitting there would
 * make the "same statement" check pass or fail on how that clause happens to
 * be punctuated rather than on whether the qualifier is actually attached to
 * the rule.
 *
