...
const SAMPLE_TASKS = [
  ...
  {
    id: 't6',
    text: 'Write integration tests — happy path',
    tag: 'focus',
    status: 'done',
    date: TODAY,
    parentId: 't1',
  ...
  {
    id: 't7',
    text: 'Write integration tests — edge cases',
    tag: 'focus',
    status: 'todo',
    date: TODAY,
    parentId: 't1',
...
async function freshPageWithData(browser, { seedData = true } = {}) {
  ...
  await page.route('**/api/calendar', (route) =>
    route.fulfill({
      ...
      body: JSON.stringify(FAKE_MEETINGS),
    })
  );
...
(async () => {
  ...
  // ss3: timeblock — switch Today's Flow to the Blocks view first
  await p2.evaluate(() => {
    window.__wl?.setFlowView('blocks');
    window.__wl?.renderTodayFlow();
    document.getElementById('todayFlowSection')?.scrollIntoView();
  });
  await p2.waitForTimeout(300);
