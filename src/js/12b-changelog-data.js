 ...
 *
 * A leaf ES module (see `LEAF_MODULES` in build-config.js) — pure literal
 * data with zero references to shared app state or to functions defined in
 * other src/js files, imported at the top of the built bundle ahead of the
 * concatenated block. That import-first placement is also what satisfies
 * 12c-startup.js's startup block, which calls mergeDevLog() (defined in
 * 12a-changelog.js) — which in turn dereferences STORE_DEV_LOG and
 * DEV_CHANGES — at top level: both are already in scope by the time any
 * concatenated file runs.
 */

export const STORE_DEV_LOG = 'wl_dev_log';
