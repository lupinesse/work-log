...
//                          (06b-hero-events.js — split out of this file)
//
// Action handlers (start/undo/done, initHero) live in 06b-hero-events.js;
// the category display/picker (_heroSetCategory et al.) lives in
// 06c-hero-category.js.
...
function _heroFillIdle() {
  ...
  if (lastEl) {
    const last = [...entries]
      ...
      .sort((entryA, entryB) => entryB.tsEnd - entryA.tsEnd)[0];
...
function _heroFillStopped() {
  ...
  if (sessEl) {
    ...
    const todayMs = entries
      .filter((en) => en.date === entry.date && en.tsEnd && en.tsEnd > en.ts)
      .reduce((sum, en) => sum + (en.tsEnd - en.ts), 0);
    sessEl.textContent =
      count > 1
...
// ── Shared helpers ────────────────────────────────────────────────────────────
// Action handlers (_heroHandleStart, _heroStartFromChip, _heroHandleUndo,
// _heroHandleDone, initHero) split to 06b-hero-events.js. Category picker
// (_heroSetCategory, _heroBindCatPicker, _heroCatSelect) split to
// 06c-hero-category.js.
...
function _heroLastNoteText(entryId) {
  const latest = logNotes
    ...
    .sort((noteA, noteB) => noteB.ts - noteA.ts)[0];
  if (!latest) return '';
...
function _heroSessionCount(entry) {
  ...
  return entries.filter((en) => en.date === todayKey && en.text.toLowerCase() === key).length;
}
